...
//            dialogue = GetDialogueFromGameObjects(Selection.gameObjects);
        ...
        {
            var zoomRect = new Rect(position.width - sidebarWidth - 160f, 50f, 140f, 100f);
        ...
        protected override void OnMouseDown(int button)
        {
            if (GetNodeBoxRect().Contains(Event.current.mousePosition))
            {
                ...
                {
                    _rightClickStartPosition = Event.current.mousePosition;
                }
            }
        }
            ...
            if (GetNodeBoxRect().Contains(Event.current.mousePosition))
            {
                if (button == 0)
                {
                    ...
                    else if (_isDraggingNode == false)
                    {
                        ...
                        if (hoveringNode == null && selectedNodeEditors.Count > 0)
                        {
                ...
                else if (button == 1)
                {
                    ...
                    var mag = Mathf.Abs((_rightClickStartPosition - Event.current.mousePosition).magnitude);
                    if (mag < 5f)
                    {
                        OnEndDrag();
