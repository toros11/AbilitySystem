...
namespace Devdog.QuestSystemPro.Dialogue.Editors
{
    public abstract class NodeEditorBase
    {
        ...
        public const int EdgeConnectorSize = 14;
        ...
        public virtual Rect[] GetEdgeConnectorRects()
        {
            var l = new Rect[Mathf.Min(node.edges.Length + 1, maxOutgoingEdges)];

            var c = node.edges.Length;
            var r = new Rect(node.editorPosition.x + nodeSize.x / 2f - EdgeConnectorSize / 2f * c, node.editorPosition.y + nodeSize.y, EdgeConnectorSize, EdgeConnectorSize);
        ...
        public virtual Rect[] GetReceivingEdgeConnectorRects()
        {
            var l = new Rect[]
            {
                new Rect(node.editorPosition.x + nodeSize.x / 2f - EdgeConnectorSize / 2f + editor.nodesOffset.x, node.editorPosition.y - EdgeConnectorSize + editor.nodesOffset.y, EdgeConnectorSize, EdgeConnectorSize)
            };
        ...
        {
            foreach (var rect in GetReceivingEdgeConnectorRects())
            {
                EditorGUIUtility.AddCursorRect(rect, MouseCursor.ArrowPlus);
                GUI.Button(rect, "", edgeConnectorStyle);
            }
        ...
        protected virtual void DrawValidation()
        {
            ...
            if (validation.validationType == ValidationType.Warning)
            {
                EditorGUI.LabelField(r, GUIContent.none, "CN EntryWarn");
            }
            else if (validation.validationType == ValidationType.Error)
            {
            ...
            {
                DrawTooltip(Event.current.mousePosition, validation.message);
            }
        }
        ...
        public virtual void DrawEdges()
        {
            ...
            var edgeConnectorRects = GetEdgeConnectorRects();
            for (uint i = 0; i < node.edges.Length; i++)
            {
                DrawEdge(i, edgeConnectorRects[i]);
            }
        }
        ...
        public void SetEdge(uint fromEdgeConnectorIndex, NodeBase connectToNode)
        {
//            Debug.Log("Connect " + node.index + " to " + connectToNode.index);
            if (fromEdgeConnectorIndex <= node.edges.Length - 1)
            {
                var existingNode = node.edges[fromEdgeConnectorIndex];
            ...
            else
            {
                ...
                l.Add(new Edge(connectToNode.index));
